...
export default {
  ...
  data() {
    return {
      ...
      front: 0, // 前
      back: 1, // 后
      interValTime: 25, // 卡片反转定时器时间
      runFlapCardAnimation: false,
      pointList: [], // 烟花 小球g个数
      runPointAnimation: false, // 烟花动画显示隐藏
      runBookCardAnimation: false, // 推荐书卡片动画
    }
  },
  watch: {
    flapCardVisible(v) {
  ...
  methods: {
    ...
    flapCardRotate() {
      ...
      let backFlpCard = this.flapCardList[this.back]
      frontFlapCard.rotateDegree += 10
      frontFlapCard._g -= 5
      backFlpCard.rotateDegree -= 10
      if (backFlpCard.rotateDegree < 90) {
        backFlpCard._g += 5
      }

      if (
        frontFlapCard.rotateDegree === 90 &&
        backFlpCard.rotateDegree === 90
      ) {
        backFlpCard.zIndex += 2
      }
      this.rotate(this.front, 'front')
      this.rotate(this.back, 'back')
      if (
        frontFlapCard.rotateDegree === 180 &&
        backFlpCard.rotateDegree === 0
      ) {
        this.next()
      }
    },
...
@import '@/assets/styles/css/flapCard';
.flap-card-wrapper {
  @include absCenter;
  width: 100%;
  height: 100%;
